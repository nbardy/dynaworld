...
from research_experiments.star_uvt_feature_tubes.projective_real_video_multiscene_extended_render_forward_residual_report import (
    DEFAULT_OUT_DIR,
    EXPECTED_STRICT_TIMING_ERRORS,
    assert_extended_render_forward_residual_report,
    run_report,
    summarize,
...
def _phase_row(step_ms: float, *, render_ms: float) -> dict[str, float]:
    backward = step_ms * 0.25
    colorize = step_ms * 0.05
    optimizer = max(0.0, step_ms - render_ms - backward - colorize)
    return {
        "step_ms": step_ms,
...
def _tile_stats(frames: int) -> dict[str, float | int | bool]:
    tile_count = frames * 32
    clipped_refs = frames * 280
    return {
        "tile_count": tile_count,
        "active_tile_count": tile_count - 2,
        "active_tile_fraction": (tile_count - 2) / tile_count,
        "raw_tile_tube_refs": clipped_refs,
        "clipped_tile_tube_refs": clipped_refs,
        "mean_tile_count": 10.0,
        "mean_active_tile_count": 10.1,
        "p50_tile_count": 10.0,
        "p95_tile_count": 16.0,
        "p95_active_tile_count": 16.0,
        "p99_tile_count": 20.0,
        "p99_active_tile_count": 20.0,
        "max_tile_count": 22,
        "overflow_tile_count": 0,
        "overflow_excess_tube_refs": 0,
        "unstable_tile_count": 0,
        "tile_capacity": 128,
    }


def _write_case(case_dir: Path, scene_id: str, frames: int, policy: str, no_first_ms: float) -> None:
    rows = [_phase_row(no_first_ms * 2.5, render_ms=no_first_ms * 1.7)]
    render_frac = 0.65 if policy == "measured" else 0.50
    for _ in range(3):
        rows.append(_phase_row(no_first_ms, render_ms=no_first_ms * render_frac))
    payload = {"step_timings_ms": rows, "tile_stats": _tile_stats(frames)}
...
def test_render_forward_residual_accepts_identical_tile_stats_misses(tmp_path: Path) -> None:
    source_path, case_dir = _write_fixture(tmp_path)

    report = run_report(source_report=source_path, case_dir=case_dir)

    assert verify_extended_render_forward_residual_report(report) == []
    assert_extended_render_forward_residual_report(report)
    assert report["summary"]["no_first_miss_pair_count"] == 2
    assert report["summary"]["all_policy_tile_stats_identical"] is True
    assert report["summary"]["workload_explains_render_forward_miss_count"] == 0


def test_render_forward_residual_rejects_stale_summary(tmp_path: Path) -> None:
    source_path, case_dir = _write_fixture(tmp_path)
    report = run_report(source_report=source_path, case_dir=case_dir)
    report["pair_profiles"][0]["render_forward_ratio"] = 99.0
...
def test_render_forward_residual_rejects_source_case_timing_mismatch(tmp_path: Path) -> None:
    source_path, case_dir = _write_fixture(tmp_path)
    report = run_report(source_report=source_path, case_dir=case_dir)
    report["pair_profiles"][0]["source_case_no_first_abs_delta"] = 1.0
...
def test_render_forward_residual_rejects_tile_workload_explanation(tmp_path: Path) -> None:
    source_path, case_dir = _write_fixture(tmp_path)
    report = run_report(source_report=source_path, case_dir=case_dir)
    report["pair_profiles"][0]["tile_stats_abs_delta"] = 2.0
    report["pair_profiles"][0]["tile_stats_equal"] = False
    report["pair_profiles"][0]["workload_explains_render_forward_miss"] = True
    report["summary"] = summarize(report)

    errors = verify_extended_render_forward_residual_report(report)

    assert any("saved tile_stats must not explain render-forward misses" in error for error in errors)


def test_render_forward_residual_rejects_dirty_pair(tmp_path: Path) -> None:
    source_path, case_dir = _write_fixture(tmp_path)
    report = run_report(source_report=source_path, case_dir=case_dir)
...
def test_saved_render_forward_residual_artifact_satisfies_contract() -> None:
