...
DEFAULT_OUT_DIR = (
    ...
    / "outputs"
    / "benchmarks"
    / "2026-05-25_star_uvt_projective_real_video_multiscene_extended_frame_scaling_diagnostic"
)
...
def verify_extended_frame_scaling_diagnostic_report(report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if report.get("status") != "ok":
        errors.append(f"status must be ok, got {report.get('status')!r}")
    if report.get("benchmark") != "star_uvt_projective_real_video_multiscene_extended_frame_scaling_diagnostic":
    ...
    if (
        ...
        or "failed strict timing verifier" not in theory_contract
        or "five-source" not in theory_contract
    ):
        errors.append("theory_contract must preserve diagnostic non-timing scope")

    source_errors = report.get("source_errors")
    if tuple(source_errors or ()) != EXPECTED_STRICT_TIMING_ERRORS:
        errors.append(f"source_errors must be exactly the expected strict timing failures, got {source_errors!r}")
    if report.get("source_status") != "failed":
        errors.append(f"source_status must remain failed to preserve the diagnostic scope, got {report.get('source_status')!r}")
    ...
    if int(source_summary.get("scene_count") or 0) < 5:
        errors.append("source diagnostic must cover at least five scenes")
    if int(source_summary.get("distinct_youtube_id_count") or 0) < 5:
        errors.append("source diagnostic must cover at least five distinct YouTube ids")
    if int(source_summary.get("row_count") or 0) < 30:
        errors.append("source diagnostic must cover at least 30 cadence/measured frame-scaling rows")
    if int(source_summary.get("frame_count_count") or 0) < 3:
        errors.append("source diagnostic must cover at least three frame counts")
    if _finite_float(source_summary.get("frame_growth_factor"), "source frame growth factor", errors) < 4.0:
        errors.append("source diagnostic must cover at least 4x frame growth")
    for key in (
    ...
    if _finite_float(source_summary.get("max_measured_vs_cadence_rebuild_ratio"), "source rebuild ratio", errors) >= 1.0:
        errors.append("source diagnostic measured rebuild ratio must stay below cadence")
    ...
    max_no_first = _finite_float(
        source_summary.get("max_measured_vs_cadence_no_first_step_ms_ratio"),
        "source max no-first ratio",
        errors,
    )
    max_growth = _finite_float(
        source_summary.get("max_measured_no_first_growth_vs_frame_growth_ratio"),
        "source max no-first/frame-growth ratio",
        errors,
    )
    if max_no_first < 1.0 and max_growth < 1.0:
        errors.append("source diagnostic unexpectedly passes timing; use the strict frame-scaling verifier instead")
    ...
    if summary.get("strict_failed_only_expected_timing") is not True:
        errors.append("diagnostic must fail only the expected strict timing gates")
    if summary.get("no_first_timing_win") is not False:
        errors.append("diagnostic must not claim a no-first timing win")
    return errors


def assert_extended_frame_scaling_diagnostic_report(report: dict[str, Any]) -> None:
    errors = verify_extended_frame_scaling_diagnostic_report(report)
    if errors:
        raise AssertionError("extended frame-scaling diagnostic failed:\n- " + "\n- ".join(errors))
...
def run_report(source_report: Path = DEFAULT_SOURCE_REPORT) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        ...
        "theory_contract": (
            ...
            "while verifying that quality/cadence/cache/support/fallback invariants remain intact."
        ),
