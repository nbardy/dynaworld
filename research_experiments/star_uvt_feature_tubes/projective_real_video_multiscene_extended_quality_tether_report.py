...
DEFAULT_OUT_DIR = (
    ...
    / "outputs"
    / "benchmarks"
    / "2026-05-25_star_uvt_projective_real_video_multiscene_extended_quality_tether"
)
...
def summarize(report: dict[str, Any]) -> dict[str, Any]:
    rows = report["rows"]
    return {
        "source_scene_count": int(report["source_trainer_matrix_summary"]["scene_count"]),
        "source_distinct_youtube_id_count": int(
            report["source_trainer_matrix_summary"]["distinct_youtube_id_count"]
        ),
...
def verify_real_video_multiscene_extended_quality_tether_report(report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if report.get("status") != "ok":
        errors.append(f"status must be ok, got {report.get('status')!r}")
    if report.get("benchmark") != "star_uvt_projective_real_video_multiscene_extended_quality_tether":
    ...
    if (
        ...
        errors.append("theory_contract must preserve the extended quality-tether scope")

    source_errors = report.get("source_trainer_matrix_verifier_errors")
    if source_errors != []:
        errors.append(f"source extended trainer matrix must verify first, got {source_errors!r}")
    source_summary = report.get("source_trainer_matrix_summary")
    if not isinstance(source_summary, dict):
        errors.append("source_trainer_matrix_summary must be an object")
    else:
        if int(source_summary.get("scene_count") or 0) < 5:
            errors.append("source extended trainer matrix must cover at least five scenes")
        if int(source_summary.get("distinct_youtube_id_count") or 0) < 5:
            errors.append("source extended trainer matrix must cover at least five source videos")
        if source_summary.get("all_measured_loss_matches_cadence") is not True:
            errors.append("source extended trainer matrix must match measured/cadence losses")
        if int(source_summary.get("max_measured_support_rebins") or 0) != 0:
            errors.append("source extended trainer matrix must have zero measured support rebins")
        if int(source_summary.get("max_measured_stale_refreshes") or 0) != 0:
            errors.append("source extended trainer matrix must have zero measured stale refreshes")

    rows = report.get("rows")
    if not isinstance(rows, list) or not rows:
        errors.append("rows must be a nonempty list")
        return errors
    expected_pairs = int(source_summary.get("scene_count") or 0) if isinstance(source_summary, dict) else 0
    if expected_pairs and len(rows) != expected_pairs:
        errors.append(f"rows must contain one cadence/measured case pair per source scene, expected {expected_pairs}")
    for row in rows:
    ...
    if summary.get("all_measured_loss_curves_match_cadence") is not True:
        errors.append("extended quality tether must match measured and cadence loss curves")
    if summary.get("all_measured_psnr_improves") is not True:
        errors.append("extended quality tether must improve measured PSNR on every pair")
    if _finite_float(summary.get("max_abs_loss_curve_delta"), "summary loss curve delta", errors) > 1.0e-8:
        errors.append("extended quality tether max loss-curve delta must stay below 1e-8")
    if _finite_float(summary.get("max_end_psnr_abs_delta"), "summary end psnr delta", errors) > 1.0e-8:
        errors.append("extended quality tether max end-PSNR delta must stay below 1e-8")
    return errors


def assert_real_video_multiscene_extended_quality_tether_report(report: dict[str, Any]) -> None:
    errors = verify_real_video_multiscene_extended_quality_tether_report(report)
    if errors:
        raise AssertionError("real-video multiscene extended quality tether failed:\n- " + "\n- ".join(errors))


def run_report(
    trainer_matrix_report: Path = DEFAULT_EXTENDED_TRAINER_MATRIX_REPORT,
    case_dir: Path | None = None,
) -> dict[str, Any]:
    source = json.loads(trainer_matrix_report.read_text(encoding="utf-8"))
    case_dir = case_dir or trainer_matrix_report.parent / "cases"
    frames = int(source["frames"])
    scenes = [str(scene["scene_id"]) for scene in source.get("scenes", [])]
    rows = [_row_from_pair(case_dir, scene_id, frames) for scene_id in scenes]
    report: dict[str, Any] = {
        "status": "ok",
        ...
        "theory_contract": (
            ...
            "measured live-cache projective-interval trainer path to the cadence full-rebuild reference by "
            "comparing saved loss curves, end losses, PSNR, and gradient-flow flags."
        ),
        "source_trainer_matrix_report": str(trainer_matrix_report),
        "source_trainer_matrix_verifier_errors": verify_real_video_multiscene_trainer_matrix_report(source),
        "source_trainer_matrix_summary": source.get("summary", {}),
...
def main() -> None:
    ...
    parser.add_argument("--trainer-matrix-report", type=Path, default=DEFAULT_EXTENDED_TRAINER_MATRIX_REPORT)
    ...
    report = run_report(trainer_matrix_report=args.trainer_matrix_report, case_dir=args.case_dir)
    if report.get("status") == "ok":
        assert_real_video_multiscene_extended_quality_tether_report(report)
