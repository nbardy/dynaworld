...
std::tuple<torch::Tensor, torch::Tensor> forward_dispatch(
...
#endif
  TORCH_CHECK(false, "gsplat_metal.forward: no backend available for device ", means2d.device());
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> backward_dispatch(
...
#endif
  TORCH_CHECK(false, "gsplat_metal.backward: no backend available for device ", means2d.device());
}

}  // namespace
}  // namespace gsplat

TORCH_LIBRARY(gsplat_metal, m) {
...
TORCH_LIBRARY_IMPL(gsplat_metal, CompositeExplicitAutograd, m) {
