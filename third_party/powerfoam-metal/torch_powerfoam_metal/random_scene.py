...
from .regular_triangulation import make_regular_triangulation_adjacency
...
def make_cech_aabb_adjacency(points: Tensor, radii: Tensor) -> tuple[Tensor, Tensor]:
    points_cpu = points.detach().cpu()
    radii_cpu = radii.detach().cpu()
    rows: list[int] = []
    offsets = [0]
    for i in range(points_cpu.shape[0]):
        ...
        overlap[i] = False
        ids = torch.nonzero(overlap, as_tuple=False).flatten()
        ids = torch.tensor(sorted(ids.tolist(), key=lambda idx: (float(dist[idx]), int(idx))), dtype=torch.long)
        rows.extend(int(v) for v in ids.tolist())
        offsets.append(len(rows))
    device = points.device
    return (
...
def make_adjacency(points: Tensor, radii: Tensor, *, mode: str, neighbors: int) -> tuple[Tensor, Tensor]:
    if mode == "cech_aabb":
        return make_cech_aabb_adjacency(points, radii)
    ...
    raise ValueError("adjacency mode must be 'cech_aabb', 'regular_triangulation', 'overlap', or 'knn'")
