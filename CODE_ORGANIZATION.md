...
Browser World Tubes seam (2026-09-06): `worldTubesMath.js` defines the source
chart and emits the analytic WGSL compiler/VJP; `worldTubesWgsl.js` owns trace
binning, ray replay, Adam and preview; `trainerWebGpuWorldTubes.js` implements
the existing worker lifecycle using only base device/readback/continuation
utilities. It does not dispatch the inherited 3DGS shaders. CPU validation in
`snapshotMetrics.js` evaluates the same UVT/conditional-depth contract. The
independent reference fixture is generated by the canonical Python SPD4
compiler under `web/dynaworld_browser_trainer/tests/fixtures/`.
...
| Paper ablation protocol | `src/train/paper_training_types.py`, `src/train/paper_training_protocol.py`, `src/train/paper_local_resources.py` | Shared typed dataset, space-time sampling, stage, and cost contracts; the opt-in local playground adds process-tree/disk/swap guards and an MPS allocator cap. Representation-specific trainers retain their kernels and model state. |
