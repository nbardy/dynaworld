...
from config_utils import apply_defaults, path_or_none, serialize_config_value
...
from pipeline.validation_media import training_preview_payload
...
from render_dispatch import pick_renderer_mode_from_config, token_layout_detail_levels
from runtime_types import SequenceData
from sequence_data import prepare_clip
from train_logging import log_wandb_run_payload_lazy
...
class MulticamRelativePoseImplicitTrainer(MulticamPrecomputedFeatureImplicitTrainer):
    ...
    def resolve_config(cls, config: dict[str, Any]) -> dict[str, Any]:
        ...
        if cfg["train"]["multires_token_detail_levels"] is not None:
            ...
            max_detail_level = token_layout_detail_levels(cfg["model"])
    ...
    def _load_checkpoint_if_configured(self) -> None:
        ...
        payload = load_checkpoint_mapping(checkpoint_path, map_location=self.device, label="Relative-pose checkpoint")
