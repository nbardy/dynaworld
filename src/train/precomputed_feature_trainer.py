...
from config_utils import apply_defaults, path_or_none
from runtime_types import SequenceData
from token_gs_trainer import Trainer as VideoTokenImplicitTrainer
...
class PrecomputedFeatureImplicitTrainer(VideoTokenImplicitTrainer):
    ...
    def resolve_config(cls, config: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            cfg["data"]["train_manifest_load_mode"] == "lazy"
            and cfg["model"]["video_encoder_backend"] != "none"
            and bool(feature_cfg["release_extractor_after_prebake"])
        ):
            raise ValueError(
                "Lazy manifest precomputed-feature training requires "
                "features.release_extractor_after_prebake=false so uncached sampled clips can be baked."
            )
        return cfg
    ...
    def training_preamble_messages(self) -> tuple[str, ...]:
        if self.model_cfg["video_encoder_backend"] == "none":
            return ("Feature cache: disabled (video_encoder_backend=none)",)
        return (
            (
                "Feature cache: "
...
__all__ = ["FEATURE_OPTION_DEFAULTS", "PrecomputedFeatureImplicitTrainer", "run_training"]
